...
export function usePlaylistEditor(api, videoById) {
  ...
  const loadPlaylistDetails = async (id) => {
    try {
      ...
      const mapped = items
        ...
        .map((item) => {
          const fullVideo = videoById.get(item.video_id) || {
            id: item.video_id,
            title: item.title,
  ...
  const addVideoToPlaylist = (videoId) => {
    ...
    const already = playlistItems.some((item) => item.video_id === videoId);
    if (already) return;

    const nextItems = [...playlistItems, { video_id: videoId, video: video }];
