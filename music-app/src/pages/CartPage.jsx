...
function CartPage() {
  ...
  const handleQtyChange = (item, next) => {
    const maxStock =
      typeof item.stock === 'number' && item.stock > 0 ? item.stock : 999;
    if (next < 1) next = 1;
    ...
    <div className="flex flex-col lg:flex-row gap-8 px-4 md:px-8 py-8">
      ...
      <div className="flex-1">
        ...
        <div className="space-y-4">
          {items.map((item) => {
            ...
            const itemTotal = (item.price_nzd ?? 0) * item.quantity;
      ...
      >
        ...
        <p className="text-[10px] text-slate-500">
          Prices are shown in NZD. Taxes and final shipping cost will be
