      ...
      <div className="bg-slate-950 rounded-lg p-4 border border-slate-800 flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-slate-500 mt-0.5">{displayGroup}</p>
          </div>
        ...
        {/* Up next */}
        <UpNextList upNext={upNext} onSelectFromUpNext={onSelectFromUpNext} />
      </div>

      {/* 下半：留言區 */}
      <div className="bg-slate-950 rounded-lg p-4 border border-slate-800 flex flex-col gap-3 max-h-[420px] overflow-y-auto">
        <h3 className="text-sm font-semibold text-slate-50">Comments</h3>
        ...
        >
          {replyTo && (
            <div className="flex items-center justify-between text-[11px] text-slate-400">
              <span>
                Replying to{' '}
                <span className="text-sky-400">{replyTo.authorName}</span>
              </span>
              <button
                type="button"
          ...
          <div className="flex items-center justify-between">
            {commentsError && (
              <p className="text-[11px] text-red-400">{commentsError}</p>
            )}
            <button
              type="submit"
