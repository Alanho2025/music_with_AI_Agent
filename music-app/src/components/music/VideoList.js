...
function VideoList({
  ...
  return (
    <div className="mt-2 flex-1 min-h-0 overflow-y-auto">
      {loading && <p className="text-xs text-slate-500">Loading videos...</p>}
        ...
        <ul className="flex flex-col gap-1">
          {videos.map((v) => {
            ...
            return (
              <li key={v.id}>
                <div
                  className={`w-full px-3 py-2 rounded-lg text-sm flex items-center border ${
                    isActive
