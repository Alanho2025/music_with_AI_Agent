        ...
        <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-wrap gap-1 text-[10px]">
                    <button
                        ...
                        className={`px-2 py-1 rounded-md border ${letterFilter === ""
                    ...
                    {letterOptions.map((ch) => (
                        <button
                            ...
                            type="button"
                            onClick={() =>
                                setLetterFilter(letterFilter === ch ? "" : ch)
                            }
                            className={`px-2 py-1 rounded-md border ${letterFilter === ch
                                    ? "bg-slate-700 border-slate-500 text-slate-50"
                                    : "bg-slate-950 border-slate-700 text-slate-400"
                                }`}
                        >
                ...
                {groupNameOptions.length > 0 && (
                    <div className="flex flex-wrap gap-2 text-[11px]">
                        <button
                            ...
                            className={`px-2 py-1 rounded-full border ${groupFilter === ""
                                    ? "bg-slate-700 border-slate-500 text-slate-50"
                                    : "bg-slate-950 border-slate-700 text-slate-400"
                                }`}
                        >
                        ...
                        {groupNameOptions.map((name) => (
                            <button
                                ...
                                type="button"
                                onClick={() =>
                                    setGroupFilter(groupFilter === name ? "" : name)
                                }
                                className={`px-2 py-1 rounded-full border ${groupFilter === name
            ...
            <div className="flex flex-col gap-2 max-h-[520px] overflow-y-auto pr-1">
                {filteredVideos.map((v) => (
                    <div
                        key={v.id}
                        className={`flex items-center justify-between px-3 py-2 rounded-md text-xs ${selectedVideoId === v.id ? "bg-slate-700" : "bg-slate-800"
                            }`}
                    >
